...
import { PriceArticle, PriceRecord, ContentIndexEntry } from '../types';
...
export const PriceWatchlist: React.FC<PriceWatchlistProps> = ({
  ...
  React.useEffect(() => {
    if (autoFocusItem && autoFocusItem.type === 'price_article') {
      const found = articles.find(a => a.id === autoFocusItem.id);
      if (found) {
        setActiveCategoryFilter('All');
        setSearchQuery(autoFocusItem.title);
        setExpandedArticleId(autoFocusItem.id);
        setTimeout(() => {
          document.getElementById(`article-${autoFocusItem.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }, 150);
        if (clearAutoFocus) {
          clearAutoFocus();
        }
  ...
  }, [autoFocusItem, articles]);

  const handleIndexItemClick = (idx: ContentIndexEntry) => {
    if (idx.contentType === 'price_article') {
      const found = articles.find(a => a.id === idx.sourceId);
      if (found) {
        setActiveCategoryFilter('All');
        setSearchQuery(idx.title);
        setExpandedArticleId(idx.sourceId);
        setTimeout(() => {
          document.getElementById(`article-${idx.sourceId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }, 100);
      ...
      <div className="glass p-5 rounded-3xl border border-stone-200/50 bg-gradient-to-br from-white/95 to-stone-50/50 dark:from-stone-900/90 dark:to-stone-950/50 shadow-sm transition-all duration-300">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-[#2d6a4f]/10 text-[#2d6a4f] rounded-2xl dark:bg-[#2d6a4f]/20 dark:text-[#b7e4c7]">
          ...
          <div className="mt-5 pt-4 border-t border-stone-200/50 dark:border-stone-800 space-y-4 animate-in fade-in duration-300">
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-white/70 dark:bg-stone-950 border border-stone-200 dark:border-stone-850 rounded-xl text-xs font-semibold focus:outline-none focus:border-[#2d6a4f]/60 dark:placeholder-stone-700 transition-all text-stone-800 dark:text-stone-100"
              />
                    ...
                    .map((idx) => {
                      ...
                      return (
                        <tr 
                          ...
                          className="cursor-pointer hover:bg-[#2d6a4f]/5 dark:hover:bg-[#b7e4c7]/5 transition-all"
