...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      // Unsubscribe existing Firestore listeners if any
      ...
      if (user) {
        ...
        // Subscribe to all cloud collections in real-time
        const unsubs = subscribeToCloudCollections(user.uid, {
          onActivitiesUpdate: (list) => {
            setActivities(prev => {
              // Only update if there is a difference to avoid state flicker
          ...
          onWatchlistUpdate: (list) => {
            setPriceArticles(prev => {
              if (JSON.stringify(prev) === JSON.stringify(list)) return prev;
              return list;
            });
          },
          onSyncStateChange: (syncing, text) => {
  ...
  const handleSaveNote = (title: string, content: string, type: NoteType, project?: string, blocks?: ProjectBlock[]) => {
    ...
    if (currentUser) {
      cloudSaveNote(currentUser.uid, targetNote);
    }
    setEditingNote(null);
  ...
  const handleAddVaultEntry = (entry: Omit<VaultEntry, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    if (currentUser) {
      cloudSaveVaultEntry(currentUser.uid, newEntry);
    }
  };
  ...
  const handleUpdateVaultEntry = (updated: VaultEntry) => {
    ...
    if (currentUser) {
      cloudSaveVaultEntry(currentUser.uid, updated);
    }
  };
  ...
  const handleAddPriceArticle = (art: Omit<PriceArticle, 'id' | 'createdAt' | 'updatedAt' | 'records'> & { initialRecord: Omit<PriceRecord, 'id'> }) => {
    ...
    if (currentUser) {
      cloudSavePriceArticle(currentUser.uid, newArticle);
    }
  };
  ...
  const handleAddPriceRecord = (articleId: string, record: Omit<PriceRecord, 'id'>) => {
    ...
    setPriceArticles(prev => prev.map(art => {
      if (art.id === articleId) {
        ...
        if (currentUser) {
          cloudSavePriceArticle(currentUser.uid, updated);
        }
        return updated;
      }
      return art;
  ...
  const handleUpdatePriceArticle = (updatedArticle: PriceArticle) => {
    ...
    if (currentUser) {
      cloudSavePriceArticle(currentUser.uid, updatedArticle);
    }
  };
    ...
    if (editingActivity) {
      setActivities(prev => prev.map(a => {
        if (a.id === editingActivity.id) {
          ...
          if (currentUser) {
            cloudSaveActivity(currentUser.uid, updated);
          }
          return updated;
        }
      ...
      if (currentUser) {
        newEntries.forEach(entry => {
          cloudSaveActivity(currentUser.uid, entry);
        });
      }
    }
  ...
  const handleAddGoal = () => {
    ...
    if (currentUser) {
      cloudSaveGoal(currentUser.uid, goal);
    }
    setNewGoalTitle('');
    setNewGoalDesc('');
    setNewGoalDate('');
  ...
  const toggleGoalCompletion = (id: string) => {
    setGoals(prev => prev.map(g => {
      if (g.id === id) {
        ...
        if (currentUser) {
          cloudSaveGoal(currentUser.uid, updated);
        }
        return updated;
      }
      return g;
      ...
      {isGoalModalOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-xl">
          <div className="w-full max-w-md bg-[#faf8f5] p-6 rounded-[32px] border border-stone-200/65 shadow-2xl">
            ...
            <div className="space-y-4">
              <input type="text" value={newGoalTitle} onChange={(e) => setNewGoalTitle(e.target.value)} placeholder="Goal Title (e.g. Clear SBI PO)" className="w-full bg-stone-50 border border-stone-200/80 rounded-xl px-4 py-4 text-sm text-[#2b2925] focus:outline-none focus:ring-2 focus:ring-[#c25e2d]/30 focus:border-[#c25e2d]" />
